...
class Controller:
    def __init__(self, api_php, repos_api, session=None):
        """
        ...
        the class instance methods for use as needed.
            ...
            :param repos_api: A formatted URL link to the GitHub API resource
                URL associated with the owner's target mirror repository,
                used for interaction with the general-purpose GitHub REST API.
    ...
    def repos_api(self):
        """
        This function serves as the primary property "getter" function used to
        externally access the ``repos_api`` instance attribute. It simply
    ...
    @repos_api.setter
    def repos_api(self, value):
        """
        This function serves as the primary property "setter" function used to
        externally set/reset new values for the ``repos_api`` instance
    ...
    def get_last_push_message(self, path, token):
        """
        ...
        by the ``path`` formal parameter existing in the target GitHub
        mirror repository. In accordance with the author's conventions, all
        ...
        request = self.session.get(url=f"{self.repos_api}/commits", params={
            "path": path
        }, headers={
    ...
    def update_via_push(self, path, token, new_content, new_message,
                        branch="master"):
        """
        The ``update_via_push`` function serves as the primary mechanism by
        which the script makes remote commits/pushes to the target GitHub
        repository mirror pointed to by ``self.repos_api``. The function makes a
        pair of REST requests, namely a ``GET`` and a ``PUT``, the first of
        ...
        url = f"{self.repos_api}/contents/{path}"
...
def main():
    ...
    username, password, owner, token = input_data
    ...
    # Link to GitHub REST API resource for owner's repository
    repos_api = f"https://api.github.com/repos/{owner}/{repo}"

    # Create new Fandom/MediaWiki/GitHub controller object
    controller = Controller(api_php, repos_api, requests.Session())
    ...
    try:
        ...
        last_commit_msg = controller.get_last_push_message(path, token)
    except (requests.exceptions.HTTPError, json.decoder.JSONDecodeError):
    ...
    for revid in add_these_revisions:
        ...
        try:
            ...
            controller.update_via_push(path, token,
